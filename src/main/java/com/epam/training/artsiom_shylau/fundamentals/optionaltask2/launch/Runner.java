...
public class Runner {
    ...
    private static final int INDEX_OF_LINE_IN_MATRIX_FOR_SORTING = 2;
    private static final int INDEX_OF_COLUMN_IN_MATRIX_FOR_SORTING = 0;
    ...
    public static void main(String[] args) {
        try {
            ...
            int[][] squareMatrix = matrixOperator.formSquareMatrixAndFillWithRandomNumbers(
                    MATRIX_SIZE, MAXIMUM_ABSOLUTE_VALUE_FOR_MATRIX_ELEMENTS);
            logger.log(Level.INFO,"Generated square matrix: \n"
                    + matrixOperator.representMatrixAsString(squareMatrix));
            matrixOperator.sortColumnsInOrderOfCertainLineElementIncreasing(
                    squareMatrix, INDEX_OF_LINE_IN_MATRIX_FOR_SORTING);
            logger.log(Level.INFO, "Sorted columns in order of line with index "
                    + INDEX_OF_LINE_IN_MATRIX_FOR_SORTING + " elements increasing: \n"
                    + matrixOperator.representMatrixAsString(squareMatrix));
            matrixOperator.sortLinesInOrderOfCertainColumnElementsIncreasing(
                    squareMatrix, INDEX_OF_COLUMN_IN_MATRIX_FOR_SORTING);
            logger.log(Level.INFO, "Sorted lines in order of column with index "
                    + INDEX_OF_COLUMN_IN_MATRIX_FOR_SORTING + " elements increasing: \n"
                    + matrixOperator.representMatrixAsString(squareMatrix));
